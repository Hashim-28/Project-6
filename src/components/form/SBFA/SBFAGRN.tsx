...
const demo = [
  {
    ...
    Show: <Link href={'/sbfa-grn-form'}><SearchIcon /></Link>,
  },
  {
    GrnNo: "10002",
    GRNDate: "2024-09-13",
...
const SBFAGRN = () => {
  const [GrnNo, setGrnNo] = useState<Checked>(false);
  const [GRNDate, setGRNDate] = useState<Checked>(false);
  ...
  const allColumns = [
    { key: 'GrnNo', name: 'GRN No' },
    { key: 'GRNDate', name: 'GRN Date' },
  ...
  const areAnyColumnsSelected = [
    GrnNo, GRNDate, ApprovedBy, Time, Show
].some(Boolean);

const selectedColumns = areAnyColumnsSelected ? allColumns.filter((column) => {
    switch (column.key) {
      case 'GrnNo': return GrnNo;
      case 'GRNDate': return GRNDate;
  ...
  return (
    ...
    <div className="material-account w-[106.2%] bg-[#1E1E2E] -ml-10 p-[61px] -mb-10 -mt-10 h-[100vh]">
      ...
      <div className="p-6 bg-[#27303D] shadow-md rounded-md mt-10 ">
       <h1 className="mb-4 text-xl text-white">SBFA GRN</h1>
            ...
            <div className="grid grid-cols-3 w-[400px] mb-10">
              <DropdownMenu>
                ...
                <DropdownMenuContent   className="w-56 text-white bg-[#1E1E2E] border-none shadow-[0px_0px_10px_5px_rgba(0,0,0,0.5)]">
                  ...
                  <DropdownMenuCheckboxItem className="focus:bg-[#3D9C75]"  checked={GrnNo} onCheckedChange={setGrnNo}>GRN No</DropdownMenuCheckboxItem>
        <DropdownMenuCheckboxItem className="focus:bg-[#3D9C75]"  checked={GRNDate} onCheckedChange={setGRNDate}>GRN Date</DropdownMenuCheckboxItem>
